   ...
   src = URL.createObjectURL(transactionProof[0]);
    ...
    const handleRecieveToken = async(e) => {
       ...
       const proof = await UploadImage(transactionProof[0])
        ...
        try {
           const res = await axios.post(`${BaseUrl}/user/transaction/user/deposit/${user._id}`, {
            ...
            <div className="container mx-auto flex max-w-3xl flex-wrap justify-center rounded-lg bg-white px-5 py-2 md:py-24">
                ...
                <div className=" md:flex">
                    <div className="mx-auto">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute z-50 rounded shadow bg-white overflow-hidden hidden peer-checked:flex flex-col w-full mt-1 border border-gray-200">
                                        ...
                                        <div className="cursor-pointer group border-t" onClick={() => setWallet({
                                            ...
                                            <button className={`w-full text-start block p-2 border-transparent border-l-4 group-hover:border-blue-600 group-hover:bg-gray-100 ${wallet.id === "LTC" ? "border-blue-950" : ""}`}>Litecoin</button>
                                        </div>
                        ...
                        <QRCode className="mx-auto mt-2 h-52 w-52 rounded-lg border p-2 md:mt-0" value={wallet.address} />

                        <div className='w-56 mx-auto'>
                            <h1 className="font-laonoto mt-2 text-center text-xl font-bold ">{wallet.name}</h1>
                            <p className="mt-1 text-center break-all font-medium text-green-500 cursor-pointer ">{wallet.address}</p>
                        </div>


                        <form className='pt-4' onSubmit={handleRecieveToken}>
                            <div className='pt-6 bg-orange-100 p-3 gap-3 rounded-lg w-52 mx-auto'>
                                <h2 className='text-sm text-blue-950 text-start'>Deposit Amount In Dollar</h2>
                                <input value={amount} onChange={(e) => setAmount(e.target.value)} min={"1"} type="number" placeholder="input amount" className='border w-full mx-auto p-2 rounded invalid:border-red-500 out-of-range:border-red-500' required />
                            </div>
                                    ...
                                    <div className="flex w-full items-center justify-center">
                                        <label className="flex h-14 w-full cursor-pointer flex-col border-4 border-dashed border-gray-200 hover:border-gray-300 hover:bg-gray-100">
                                            <div className="mt-4 flex items-center justify-center space-x-1">
                                                ...
                                                <p className="font-laonoto text-sm tracking-wider text-gray-400 group-hover:text-gray-600">upload</p>
                                            </div>
                                            <input  onChange={(e) => setTransactionProof(e.target.files)} type="file" className="opacity-0" required />
                                        </label>
                                    </div>
               ...
               />
                            ...
                            {!isLoading && <button className="mx-auto block rounded-md border bg-blue-500 px-6 py-2 text-white outline-none">Submit</button>}
                        ...
                        <div className='mt-4 bg-blue-500 rounded-md p-6 text-white'>
                            <p>Send only {wallet?.name} ({wallet?.id}) to this
                                address.</p>
                    ...
                    <div className="mt-8 max-w-sm md:mt-0 md:ml-10 md:w-2/3">
                        ...
                        <div className="relative flex pb-12">
                            ...
                            <div className="flex-grow pl-4">
                                ...
                                <p>By direct transfer from your account to your <b>Wallet Address</b>.</p>
                            </div>
                        </div>
